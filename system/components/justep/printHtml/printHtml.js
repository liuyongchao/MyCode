...
define(function(require) {
	...
	var executePrintout = function(model, formXID, isPreview, options) {  
	    ...
	    writeDoc.open();
	    writeDoc.write('<!DOCTYPE html> '+"<html class='"+getHtmlClass()+"'>" + getHead(settings) + "<body "+ ((justep.Browser.IE&&isPreview)?"onload='try{window.loaded = true;wb.execWB(7,1);}catch(e){alert(\"ActiveX未能正常加载\");}'":"onload='window.loaded = true;'") + ">" + getActiveX(isPreview) + getHtml(model, formXID) + (settings.appendHtml || '') + "</body>" + "</html>" );
	    writeDoc.close();
