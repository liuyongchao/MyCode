    ...
    var ScrollView = ViewComponent.extend({
        ...
        render:function(){
			...
			var defaultConfig = {
				...
				onScrollEnd: function (scrollType) {
					return self.fireEvent('onScrollEnd',{source:self,scrollType:scrollType});
				},
		...
		pullUp : function(loadCallback) {
			...
			this._puScrollMove = function(){
				 ...
				 * 
				 * scrollLite
				 * 		滚动区域为x-scroll部分 里面包含  
				  			x-pull-down x-scroll-content x-pull-up三部分	
				  		maxScrollY 是按照可以滚动的最大距离计算 没滚动条时候为0  有滚动条时候为 负值
				  		y  是滚动出去的距离 滚上去后 y为复制 所以初始化时候 y本应该为0 但是上面让出来了一个50的下拉刷新 提示框 所以值为 -50
				  		下拉刷新完全拉出来后为0
				  	iScroll	
				  	maxScrollY iscroll 在没有撑出滚动条的时候 值为正值是说还有多少距离才能滚动
				 */
