	...
	Model.prototype._refreshBarcode = function(wizard) {
		...
		if (this.ipaExsist) {
			if (this.appEngine.webIDE) {
				this.iosURL = "ip="+this.ip+"&platform=webideIOS&projectName=" + config.projectName + '&appName=' + config.appName + '&packageName=' + config.packageName;
				this.comp('iosQR').set({
					"value" : "http://" + location.host + require.toUrl("$UI/system/deploy/package/app.w") + "?" + this.iosURL
				});
				return;
			}
			this.appEngine.startIpaService(this.ip, config.projectName);
			this.iosURL = "http://" + this.ip + ":" + point + "/intro.html";
			this.comp('iosQR').set({
				"value" : this.iosURL
			});
		}
