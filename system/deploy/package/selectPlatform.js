define(function(require) {
	...
	Model.prototype.finish = function() {
		...
		if (this.validate()) {
			...
			if (!config.useProxyBuilderServer) {
				...
				if (config.platform && (config.platform.indexOf('ios') >= 0)) {
					dtd.resolve("true");
					self.getParent().openPage({
						id : "waittingDialog",
						url : "$UI/system/deploy/package/waittingDialog.w",
				...
				function checkStatus() {
					if (i > 10) {// test
						...
						var response = $.ajax({
							...
							processData : false,
							type : 'GET',
							url : require.toUrl("$UI/system/deploy/package/test.j?appBuilderServer=" + self.appEngine.getAppBuilderServerUrl() + "&useProxyBuilderServer="
									+ config.useProxyBuilderServer),
	...
	Model.prototype.validate = function() {
		...
		if (!result) {
			...
			config.proxyBuilderServer = this.appEngine.getAppBuilderServerUrl();
